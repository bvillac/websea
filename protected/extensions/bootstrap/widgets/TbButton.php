...
class TbButton extends CWidget {
     ...
     * @var string para remover clases que no se desean que esten en el elemento. 
     */
    public $removeClass;
    ...
    public function init() {
        ...
        /* cambio Byron 27-08-2012
         * Permitir ruta de imagenes en los botonos
         */
        if (isset($this->ruta)) {
            if (strpos($this->ruta, 'ruta') === false)
            //$this->ruta = 'ruta-'.implode(' ruta-', explode(' ', $this->ruta));
            //$this->label = '<i class="'.$this->ruta.'"></i> '.$this->label;
                $this->label = '<img src="' . $this->ruta . '" title="' . $this->tooltip . '" > ' . $this->label;
//                        <a href="#" rel="tooltip" title="First tooltip">
        ...
        /* removiendo clases que no se desea : Eduardo Cueva */
        if (isset($this->removeClass)) {
            $rmClass = explode(' ', $this->removeClass);
            $htmlClass = $this->htmlOptions['class'];
            for ($i = 0; $i < count($rmClass); $i++) {
                $htmlClass = str_replace($rmClass[$i], "", $htmlClass);
            }
